...
def _run_pytest_collect(pytest_args):
    ...
    if exit_code == EXIT_INTERRUPTED:
        # pytest raises EXIT_INTERRUPTED on *both* keyboard error and parse
        # errors
...
def merge_config(args, pytest_args, silent=True, verbose=False):
    ...
    if not config.has_section('python-watch'):
        return True
    ...
    for cli_name in args:
        ...
        # Find config option
        if not config.has_option('python-watch', config_name):
            continue

        # Merge config option using the expected type
        if isinstance(args[cli_name], list):
            args[cli_name].append(config.get('python-watch', config_name))
        elif isinstance(args[cli_name], bool):
            args[cli_name] = config.getboolean('python-watch', config_name)
        else:
            args[cli_name] = config.get('python-watch', config_name)
