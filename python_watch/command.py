...
Usage: pyw [options] [--pattern <pattern>] [--ignore <pattern>...] (<file>)

Options:
  --pattern <pattern>  Pattern to watch for change
  --ignore <pattern>    Ignore pattern from being watched and during
...
doc = '\n\n\n'.join(__doc__.split('\n\n\n')[1:])
version = 'python-watch ' + __version__
...
def main(argv=None):
    ...
    # Get paths file to execute on event
    script = args['<file>']

    python_args = []
    # Adjust python and --collect-only args
    for ignore in args['--ignore'] or []:
        python_args.extend(['--ignore', ignore])
    if args['--config']:
        python_args.extend(['-c', args['--config']])

    # Adjust pytest args
    if args['--pdb']:
        python_args.append('--pdb')
    ...
    return watch(entry=script,
                 ignore=args['--ignore'],
                 extensions=extensions,
                 beep_on_failure=not args['--nobeep'],
                 auto_clear=args['--clear'],
                 wait=args['--wait'] or '--pdb' in python_args,
