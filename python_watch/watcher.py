...
class EventListener(PatternMatchingEventHandler):
    """
    Listens for changes to files and re-runs script after each change.
    """
    ...
    def __init__(
            ...
            ignore_patterns=[],
            event_queue=None):
        super().__init__(patterns=patterns or ['*.py', '*.pyc'],
                         ignore_patterns=ignore_patterns or ['version.py'],
...
def _get_python_runner(custom, file=""):
    if custom:
        return custom.split(' ')

    if file == "":
        raise ValueError('File must be a non void string')
...
def _show_summary(argv, events, verbose=False):
    command = ' '.join(argv)
    def bright(arg): return STYLE_BRIGHT + arg + Style.RESET_ALL
    def highlight(arg): return STYLE_HIGHLIGHT + arg + Style.RESET_ALL
...
def watch(entry, patterns=[], ignore=[], extensions=[], beep_on_failure=True,
          auto_clear=False, wait=False, beforerun=None, afterrun=None,
          onpass=None, onfail=None, onexit=None, runner=None, spool=None,
          poll=False, verbose=False, quiet=False, python_args=[]):
    argv = _get_python_runner(runner, entry) + (python_args or [])

    if not entry:
        raise ValueError("Must specify a pythin script to execute")

    # Setup event handler
    event_listener = EventListener(
        extensions, patterns=patterns, ignore_patterns=ignore)

    # Setup watchdog
    path = '.'
    observer = Observer()
    observer.schedule(event_listener, path, recursive=True)
    observer.start()
